...
set runpath=H:\TOols\Rolan\BaopoTools\弱口令检测\john\run

rem 定义字典文件名称，如字典未放在工具目录下则需添加路径
set zd=H:\TOols\Rolan\BaopoTools\弱口令检测\dict\passwordyyds.dict
::set zd=jiancha.txt
...
echo ###请选择扫描路径###
set /p f=你的选择:
for /r %f% %%a in (*.*) do echo %%~dpnxa >>%runpath%\filelist.txt
for /f "delims=" %%s in ("%f%") do (
set pathname=%%~ns
)
rem 设置结果存放路径result
set newpath=H:\TOols\Rolan\BaopoTools\弱口令检测\john\result
...
call set a=%%con!j!%%
echo 正在扫描!a!:
echo %%~dpnxi : >> %newpath%\%pathname%-single-%D%.txt
...
call set a=%%con!j!%%
echo 正在扫描!a!:
echo %%~dpnxi : >> %newpath%\%pathname%-dic-%D%.txt
...
call set a=%%con!j!%%
echo 正在扫描!a!:
echo %%~dpnxi : >> %newpath%\%pathname%-ora-%D%.txt
...
echo %%~dpnxi : >> %newpath%\%pathname%-form-%D%.txt
...
echo %%~dpnxi : >> %newpath%\%pathname%-mix-%D%.txt
